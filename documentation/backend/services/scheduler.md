...
4. **retry_missing_torrents** - Default: daily midnight, re-searches 'awaiting_search' status (limit 50), handles both audiobook and ebook requests, enabled by default. Also recovers requests stranded in 'downloading' (monitor bumps updatedAt every ≤5 min, so >2h without an update = dead monitor job) by resetting to 'awaiting_search' and re-triggering search. 'processing' is excluded (long chapter-merges legitimately run for hours without an update).
